...
public class AdministrationShopTogglerService
{
    public readonly BeService beService;
    public AdministrationShopTogglerService(BeService srv)
    {
        beService = srv;
    }
    ...
    public bool SetActive(Guid Id, bool active)
    {
        bool output = false;
        Shop shop = AllShops().Where(e=>e.Id.Equals(Id)).First();
        if (shop is not null)
        {
            shop.Active = active;
            beService.DbContext.Update(shop);
